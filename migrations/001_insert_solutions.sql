-- Migração para inserir as soluções iniciais
-- Esta migração usa ON CONFLICT para evitar duplicações

-- Criar a tabela solutions se não existir
...
-- Inserir ou atualizar as soluções
...
VALUES
  ...
  ('bot-comentarios', 'Bot para Interações em Comentários', 'Responde automaticamente comentários no Instagram/Facebook com base em palavras-chave.', 'atendimento', 'Zap', 'bg-yellow-500/10 text-yellow-500', false, true, false, null),
  ('confirmacao-consulta', 'Confirmação de Consulta', 'Lembra o cliente da consulta com botão para confirmar ou reagendar.', 'agendamento', 'Clock', 'bg-indigo-500/10 text-indigo-500', false, true, false, null),
...
-- Conceder permissões para a tabela solutions
